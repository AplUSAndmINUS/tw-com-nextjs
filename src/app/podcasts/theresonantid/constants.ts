export const TRI_LINKS = {
  instagram: 'https://www.instagram.com/theresonantidentity/',
  twitter: 'https://x.com/theresonantid',
  tiktok: 'https://www.tiktok.com/@theresonantid',
  podcast: '/podcasts/theresonantid/', // setting as the main podcast page for now since it links to all platforms
  about: 'https://www.theresonantidentity.com/',
  website: 'https://www.theresonantidentity.com/',
  youtube: 'https://www.youtube.com/@theresonantidentity',
  facebookGroup: 'https://www.facebook.com/groups/theresonantidentity',
  facebookPage: 'https://www.facebook.com/theresonantidentity',
} as const;

/** Handles used when querying the YouTube API for TRI + Terence channels. */
export const TRI_YOUTUBE_CHANNEL_HANDLES = [
  'theresonantidentity',
  'TerenceRWaters',
] as const;
